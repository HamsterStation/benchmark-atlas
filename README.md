...
请求至少间隔 3.1 秒，有超时、响应大小限制、有限重试与退避。遇到 **HTTP 429** 立即停止本轮所有 arXiv 查询，不连续重试；将 `arxiv_retry_after` 写入持久状态，默认冷却 15 分钟。`Retry-After` 支持秒数与 HTTP 日期，官方要求更久时按更久的期限等待，不截短为 60 秒。冷却中的后续调度跳过，手动重跑也不能绕过冷却。页返回空但总数尚未覆盖时判失败，不推进成功游标。不并发运行本地采集进程；CI 使用共用 concurrency 锁避免多个写者。基础 ID 去重，版本更新进入同一 ID 的新草稿。已有成功结果与版本相同，不重复调用模型。

先把取得的材料放入持久队列，再提交游标。arXiv 出错或本轮分页未完成时不调用模型、退出码非零并禁止部署；后续补跑从固定窗口与游标继续。单篇模型失败不会被游标推进吞掉，失败条目保留；模型异常退避 6 小时，每版本总尝试上限默认 3 次。达到上限后保留 `retry_exhausted`，修好服务后维护者可在 state 分支将该条目的 `attempts` 设为 0、`next_attempt_at` 设为 null 再重试。不要删除队列来“消除”错误。
...
`collect.yml` 计划在 **北京时间 10:20** 主运行，并在 **11:20、14:20、18:20** 检查补跑。UTC 对应 02:20、03:20、06:20、10:20。`schedule-check` 先读取 `atlas-state`：当天采集与发布均成功则跳过；采集成功但构建、Pages 或 review PR 发布失败，只重试发布，不重复调用 arXiv 或模型；未完成采集则从原队列恢复。并发锁覆盖检查、采集和部署，补跑共用每天最多 2 篇与模型调用预算，不会每次再收 2 篇。
...
`last_collection_attempt` 保存最近尝试时间、是否完成、错误类型和安全的 HTTP 状态码；不保存原始响应或凭据。`last_successful_collection_at` 表示 API 分页完成，`last_publishable_collection_at` 仅在分页、模型处理均无错误时前进，`last_successful_deployment_at` 仅在 Pages 成功后前进。构建只接受生成时间不晚于可发布时间的自动草稿，防止失败采集中的新草稿被后续代码 push 顺带发布。升级旧状态时使用原有成功采集时间兼容已有线上条目。
