...
站点通过 GitHub Actions 构建并发布到 Pages；部署结果见仓库的 **Deploy benchmark site** 工作流。当前按维护者要求采用 **auto 自动收录模式**，每天 **北京时间 10:20** 主采集，**11:20、14:20、18:20** 自动补跑；当天成功后跳过后续补跑，每天最多处理 2 篇候选。通过证据筛选、字段校验和构建测试的条目直接并入网站主列表，统一排序与搜索，无需逐篇批准 PR。页面只保留主题分类、中文简介、官方资源和复现方法，不展示核查/复现状态、状态筛选或空运行记录。
...
当前网站只展示 **2023 年起首发** 的论文，优先 Agent、软件工程、工具调用和通用模型能力评测，排除医学、生物医学、临床专题。2022 年及更早的 5 篇初始论文和 PhenoNormBench 已从首页、搜索索引和详情页移除；原始资料留档，未删除人工笔记。年份按 `publishedAt` 判断，新版本更新日期不能使旧论文重新上线。

本次新增 **SWE-Gate（2026-09-03）** 和 **APIFlow-Bench（2026-08-29）**，已核对 arXiv 摘要和固定 commit 的官方 README，附中文简介、指标、资源入口和有出处的运行方法。只读取文档，没有执行第三方仓库代码。

继续补充 **CordisBench（2026-09-01，v1）** 和 **DuMateBench（2026-08-27，v1）**：前者评测模型对 Agent 框架组件生命周期的推理，后者评测真实办公流程中的 Agent 交付能力。两篇均已核对原论文摘要、固定 commit 的官方 README 和数据入口，附来源明确的环境、命令与指标。网站目前共 10 篇；手动追加不改变每日自动收录上限。采集队列遇到已手动收录的相同或更早版本时跳过模型调用，后续新版本仍正常跟踪。

以下为初始 10 篇的历史来源清单，前 5 篇不再公开展示。已于 **2026-09-10** 联网核对 arXiv API 的原论文元数据与摘要；9 个官方仓库 README 读取并固定至完整 commit。GAIA 的官方资源组织入口来自原论文摘要，数据卡直接读取返回 HTTP 401，具体代码和下载步骤保持待核查。
...
每条 `sources` 保存版本、核查日期与核查范围。`source_verified` 表示这些来源已核对，不表示维护者人工审核，更不表示亲测。这些状态仅作为数据校验和维护记录保存，页面与公开搜索索引不展示。初始 10 篇条目均未实测，其中 9 篇有官方文档入口。摘要和官方 README 整理不能冒充全文解读。复现方法展示文档链接、已有说明及带来源版本的命令；没有官方步骤的条目提供原论文入口，不补猜命令。
...
采用 **最新研究优先、证据达标再收录** 的筛选：2023 年起的经典论文继续保留，日常主要寻找新提出的 Agent、软件工程与通用模型 Benchmark、评测环境、评测数据集或评测框架。首页默认按论文首发日期倒序，仍可切换最新收录和最近更新。仅使用已有基准评估新方法的论文，以及没有新基准贡献的综述，默认排除。机构名、作者名、GitHub star、摘要中的宣传用语不加分；会议或期刊信息只作为未核实声明保存，不冒充已确认录用。
...
**每天最多处理 2 篇候选，不凑数。** `config/quality.json` 中的 `scope` 是采集和网站共用的主题与年份规则；医学排除先检查标题与基准贡献句，避免因泛化讨论偶然提及医学而误排。网站构建也对恢复的历史草稿检查标题、任务形式和年份，防止旧条目再次上线。规则属于静态配置，外部材料不能修改。`max_candidates_per_run` 和 `max_candidates_per_day` 均为 2，按 `Asia/Shanghai` 自然日计数。每日 ID/版本名额写入持久 state，手动重跑共用上限，dry-run 不消耗名额。按原始首发时间划分 **近 7 天 → 近 30 天 → 近 90 天** 三档；同档先看 Agent / 编程方向，再按证据完整度、首发时间排序。旧论文仅更新版本不会挤进新论文档。超出 90 天的未收录材料保留队列且不占名额；2023 年起的已收录经典论文的新版本单列在新论文之后处理。时间档每轮重新计算，不因缓存证据而冻结。未处理候选下次继续。无模型时只保存元数据和证据，简介留空；后续生成也占当日名额。模型只对达到门槛的候选工作，已有成功结果不会因重复采集再次生成。auto 模式通过校验后直接部署，review 模式则等待人工提升和合并。
...
每页最多 200 条，前一个响应完成后至少间隔 10 秒，保留超时、响应大小限制与有限重试。当天已完成的同一检索不重复扫描，未完成窗口继续分页；保留 7 天重叠检查窗口。遇到 **HTTP 429** 立即停止所有 arXiv 查询，持久记录 `arxiv_retry_after` 与连续失败次数，冷却从 1 小时逐次倍增到最多 24 小时。`Retry-After` 支持秒数与 HTTP 日期，官方要求更久时等待更久。冷却期间不请求 arXiv，手动重跑也不能绕过。页返回空但总数尚未覆盖时判失败，不推进成功游标。CI 使用共用 concurrency 锁避免多个写者；基础 ID 去重，版本更新单独跟踪。

发现与整理分成两个独立步骤。`--discover-only` 只抓取元数据、保存队列和游标，不调用模型；所有检索分页完成后才把材料哈希写入 `ready_queue`。接口失败或分页未完成时，该步骤退出码非零，失败批次不进入可处理快照。`--queue-only` 不访问 arXiv，只处理快照中哈希匹配且快照完成距今不超过 7 天（`queue_cache_max_age_days`）的材料；按现有质量门槛和每日两篇上限生成简介，通过校验与构建后可独立发布。不存在完整快照、缓存过期、仅有新版本的部分材料时停止发布，绝不把缓存整理当成今天发现了新论文。

升级旧状态时，只有原始材料哈希与已有 triage 完全一致、且 triage 时间不晚于上次成功采集时间，才允许迁入 `ready_queue`；无证明的队列继续等待新一轮完整发现。模型失败或缺少 Key 时保留条目、不推进可发布时间；模型异常退避 6 小时，每版本总尝试上限默认 3 次。达到上限后保留 `retry_exhausted`，修好服务后维护者可在 state 分支将该条目的 `attempts` 设为 0、`next_attempt_at` 设为 null 再重试。不要删除队列来“消除”错误。

离线预览缓存整理（不请求 arXiv、不调用模型、不写资料）：

```bash
.venv/bin/python -m collector.collect --queue-only --dry-run --report work/queue-preview.json
```

GitHub 手动运行可勾选 `queue_only=true`；默认仍先尝试发现新论文。Actions artifact 分别保存 `discovery-report.json` 和 `collection-report.json`。发现失败会产生明确 warning 和错误报告，缓存整理成功不会将发现失败伪装为成功采集。
...
每日模型调用上限 6（按 UTC 计数，独立于北京时间每日 2 篇名额），输入上限 12,000 字符，每轮重试上限 1，每个版本总尝试上限 3，均可配置。失败调用也占配额。CI 会在模型请求前把配额预留推送到持久 state 分支；持久化失败就停止，不在未知配额状态下继续请求。模型返回格式或服务不兼容时保留待审核元数据，错误只记录类型、HTTP 状态码与固定白名单内的参数提示，不记录请求头或原始响应。

连接异常时，可手动运行 **Diagnose model connection** 工作流。它复用已有 Secret、模型配置和近期完整采集的论文材料，只调用一次模型，验证返回字段及中文简介；先持久预留同一每日预算，不重试、不写入草稿、不发布网站。安全结果保存在该次运行的摘要和 `model-diagnostic` artifact 中，包含成功与否及 HTTP 状态码，不包含密钥、服务原始错误或生成内容。诊断与生产采集共用并发锁；没有可用材料或预算时不调用模型。诊断成功仅说明当次连接及返回格式正常，不等于全文内容已核实。
...
`collect.yml` 计划在 **北京时间 10:20** 主运行，并在 **11:20、14:20、18:20** 检查补跑。UTC 对应 02:20、03:20、06:20、10:20。`schedule-check` 先读取 `atlas-state`：当天整理与发布均成功则跳过；整理成功但构建、Pages 或 review PR 发布失败，只重试发布；当天发现完成但整理未完成，或接口在冷却中且存在旧材料时，仅运行队列整理；其余情况先发现再整理。若今天使用缓存完成发布，后续补跑跳过，下一天再尝试发现新论文。并发锁覆盖检查、发现、整理和部署，所有运行共用每天最多 2 篇与模型调用预算。

支持 `workflow_dispatch` 手动触发，默认 `dry_run=true`。默认 `COLLECTION_ENABLED` 未设置时定时任务跳过，当前仓库已开启。生产采集只在默认分支运行。GitHub [官方文档](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule) 明确说明 schedule 可能延迟，高负载时可能丢弃；因此单次 cron 无法保证精确时间，补跑时段用于减少单次漏触发的影响。所有时段都由 GitHub 调度，不能承诺独立于平台的可用性。

`last_collection_attempt` 保存最近发现尝试的时间、完成情况及安全错误信息，缓存整理不会覆盖它。`last_successful_collection_at` 仅在 API 分页完成后前进；`last_processing_attempt` 与 `last_successful_processing_at` 单独记录整理情况；`last_publishable_collection_at` 在符合快照边界的整理成功后前进；`last_successful_deployment_at` 仅在 Pages 成功后前进。来源说明记录采集快照完成时间，生成时间单独保留。构建仍限制自动草稿生成时间，升级旧状态仍兼容已有线上条目。
...
发现失败批次不会发布；独立的缓存整理可以在发现失败期间成功发布旧快照里的合格候选。持久化、整理、校验或构建失败均不进入部署 job，现有线上站点保留。成功采集时间仅在所有检索窗口完成后更新，单页预算中断不算整个检索完成。部署时间只在 deploy-pages 成功后记录到 `atlas-state:automation/deployment.json`。网站显示构建时已知的成功记录，最新值以该分支记录和 Pages 运行结果为准。若部署成功但记录写入失败，工作流仍会报错。缓存候选有限且有 7 天有效期；不能保证上游持续不可用时无限期更新。
...
此前主题与年份调整通过 **10 个 TypeScript 测试、57 个 Python 测试、12 个桌面/手机页面测试**、类型检查及生产构建，验证旧年份和医学条目不再生成详情路径、新论文的简介和官方方法正常展示。页面测试不复用已有服务，避免误测旧构建；本机 4321 端口已有开发服务时，可运行 `PLAYWRIGHT_PORT=4397 npm run test:pages`。联网 dry-run 完整读取 16 页，新增发现 15、规则复筛 152、跳过 524、版本更新 0、失败 0；证据达标 15、待补证据 914、范围排除 227。模型调用 0，状态、草稿与人工笔记的文件哈希保持一致；当日自动处理名额已用 2/2，未再生成草稿。本次两篇来源核对条目通过手动入口补充，未修改自动配额。
...
已检查 GitHub 默认分支、工作流启用状态、Actions 权限、开关与运行账号：配置均正常，10:20 没有生成 schedule 事件；GitHub 未提供该次投递缺失的内部原因，不能把它断言为权限错误或 cron 写错。单次调度缺少补偿已改为主任务加 3 个补跑时段。

本次本地验证通过 **71 个 Python 测试、11 个 TypeScript 测试、12 个桌面/手机页面测试**、Astro 类型检查与生产构建、actionlint，以及新增条目的隔离构建测试。覆盖漏触发后补跑、成功后跳过、发布失败只重试发布、北京时间跨天、429 冷却与过期恢复、Retry-After 秒数/日期、失败批次不调用模型、未完成分页禁止发布。真实状态的冷却 dry-run 不发网络/模型请求、不改资料；使用包含今天失败草稿的真实状态构建仍只发布原有 10 篇。手动触发测试与真实 schedule 投递应分别记录，不能用 workflow_dispatch 冒充定时触发验证。
