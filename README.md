...
站点通过 GitHub Actions 构建并发布到 Pages；部署结果见仓库的 **Deploy benchmark site** 工作流。当前按维护者要求采用 **auto 自动收录模式**，每天 **北京时间 10:20** 采集、最多处理 2 篇候选。通过证据筛选、字段校验和构建测试的条目直接并入网站主列表，统一排序与搜索，无需逐篇批准 PR；机器检查不冒充人工审核或实测复现。
...
| `auto`（当前采用） | 通过 schema 和内容边界校验的新增模型草稿进入索引，页面标注“自动收录”，与其他论文一起排序、搜索和筛选；不设置单独待审核区。元数据空简介、uncertain 与 uses_benchmark 不发布。该次采集工作流直接构建、上传 Pages artifact 并部署，不依赖机器人 push 触发另一个工作流。 |
